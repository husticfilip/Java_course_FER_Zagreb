...
public class SmartScriptParser {
	...
	private void handleForLoop() {
		...
		if (token.getType() != SmartScriptTokenType.VARIABLE) {
			...
			if (forElements.size() == 2) {
				...
				Element start = testIsVariableNumberOrString((SmartScriptToken) forElements.get(0));
				Element mid = testIsVariableNumberOrString((SmartScriptToken) forElements.get(1));
				Element end = testIsVariableNumberOrString((SmartScriptToken) forElements.get(2));
				stack.push(new ForLoopNode(variable, start, end, mid));
			} else {
	 ...
	 */
	private Element[] getEchoNodeArray(ArrayIndexedCollection collection) {
