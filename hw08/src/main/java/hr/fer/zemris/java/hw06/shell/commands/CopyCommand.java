...
public class CopyCommand implements ShellCommand {
	 ...
	 */
	private final String description = "Copy command is used to copy out content of input file%n" + "to ouput file.%n"
			+ "arg1->path to input file arg2->path to output file%n"
			+ "If arg2 is directory copy of file has the same%n" + "name as original file.%n"
			+ "If output file does not exist new file with given name%n" + "is made.";
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		try {
			inputPath = getPath(env, lexer.nextToken().getValue());
			outputPath = getPath(env, lexer.nextToken().getValue());
	...
	private boolean askToOverwriteIfNeeded(Environment env, Path outputPath) {
		...
		if (Files.isRegularFile(outputPath)) {
			env.writeln("File at given destination already exist. Do you want to overwrite it?(yes/no)");
	...
	/**
	 * Method resolves given path. If path is absolute that absolute path is
	 * returned. Else if path is relative it is resolved with working directory of
	 * environment.
	 * 
	 * @param env        working Environment.
	 * @param stringPath absolute or relative path.
